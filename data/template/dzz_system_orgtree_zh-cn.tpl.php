<?php if(!defined('IN_DZZ')) exit('Access Denied'); /*a:3:{s:64:"D:\phpstudy_pro\WWW\dzzoffice\/./dzz/system/template/orgtree.htm";i:1586239670;s:85:"D:\phpstudy_pro\WWW\dzzoffice\/./core/template/default/common/header_simple_start.htm";i:1586239670;s:83:"D:\phpstudy_pro\WWW\dzzoffice\/./core/template/default/common/header_simple_end.htm";i:1586239670;}*/?>
<!DOCTYPE html>
<html>
<head>
...
var showjob=parseInt('<?php echo $_GET['showjob'];?>')>0?1:0;
var stype='<?php echo $_GET['stype'];?>'?parseInt('<?php echo $_GET['stype'];?>'):0;//0:可以选择机构和用户；1：仅选择机构和部门：2：仅选择用户
var range='<?php echo $_GET['range'];?>'?parseInt('<?php echo $_GET['range'];?>'):0;//0:机构和用户；1：仅机构和部门：2：仅群组
...
<div  class="orgtree-search">
        <a href="javascript:;" class="search" onclick="jstree_search();return false" title="搜索"><i class="dzz dzz-search"></i></a>
        <a href="javascript:;" class="delete" onclick="jstree_search('stop');return false" title="关闭搜索框"><i class="dzz dzz-close"></i></a>
...
var self=this;
jQuery.post(DZZSCRIPT+'?mod=system&op=orgtree&do=orgtree',{'id':node.id,'nouser':nouser,'moderator':moderator,'zero':'<?php echo $zero;?>','stype':stype,'range':range,'showjob':showjob},function(json){
cb.call(this,json);
},'json');
}
  },
...
if((stype==0 || stype==2) && orgids[i].indexOf('uid_')===0){ //用户
...
if((jQuery.isNumeric(sels[i]) || sels[i]=='other')){//是机构或部门
if(stype==2) continue;
var path=node.text;
