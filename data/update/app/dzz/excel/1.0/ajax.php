...
if ($do == 'mydoc') {//我的文档
    ...
    $options = array('accept_file_types' => '/\.(xls|xlsx)$/i',
	...
	if ($tpl) {
		...
		$name = censor(trim($_GET['name']) ? trim($_GET['name']) : lang('new_excel'));
		if ($perm) {
	...
	if ($operation == 'setbase') {
		C::t('user_setting')->update(array('doc_excel_tplhide' => $tplhide, 'doc_excel_createtype' => $createtype, 'doc_excel_savepath' => $savepath, 'doc_excel_openappid' => $appid), $_G['uid']);
		...
		C::t('user_setting')->update_by_skey('doc_excel_showtype', $showtype, $_G['uid']);
