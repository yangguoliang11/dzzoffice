...
if ($do == 'cat_rename') {
	...
	$cid = intval($_GET['cid']);
	$tid = intval($_GET['tid']);
	$cat = C::t('doc_template_cat')->fetch(array('cid' => $cid, 'type' => 2));
	...
	if(submitcheck('addtpl')){
		$cid = intval($_GET['cid']);
		if (!C::t('doc_template_cat')->fetch(array('cid' => $cid, 'type' => 2))) exit(json_encode(array('code' => 400, 'message' => lang('chose_cat_error'))));
		$setarr = array(
					'title' 		=>		 $_GET['name'],
					'attach'		=>		 $_GET['aid'],
