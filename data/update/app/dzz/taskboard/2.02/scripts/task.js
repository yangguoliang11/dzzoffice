...
function task_share_to_desktop(tbid,taskid){
	jQuery.getJSON(DZZSCRIPT+'?mod=system&op=dzzcp&do=linktodesktop&link='+encodeURIComponent(url),function(json){
		if(json.error){
			alert(json.error);
		}else{
			try{
...
function board_share_to_desktop(tbid){
	var url=SITEURL+MOD_URL+'&op=list&tbid='+tbid;
	 jQuery.getJSON(DZZSCRIPT+'?mod=system&op=dzzcp&do=linktodesktop&link='+encodeURIComponent(url),function(json){
		  if(json.error){
		  	alert(1)
			  alert(json.error);
			  ...
			  try{
				  parent._ico.createIco(json);
				 parent.showmessage('成功创建桌面快捷方式','success',3000,1);
			  }catch(e){
				  alert('成功创建桌面快捷方式');
			  } 
		  }
	...
	if(flag=='catlist'){
		if(layout>0){
			...
			jQuery('#catlist_container').dzzdragersort_catlist({
				'scrollContainer':jQuery('#taskboard_container'), //滚动层
				'contentContainer':jQuery('#catlist_container'), //内容层
				'hoder_div_css':'margin:25px 5px 5px 0;float:left;background-color:RGBA(0,0,0,.15);border-radius: 5px;',
				'width_correct':15,
				'height_correct':15,
				'itemselector':'.catlist',
				'scrollspeed':20,
...
function layout_catlist(el){
	...
	var catlistHeight=jQuery('.bs-main-container').height()-parseInt(jQuery('.catlist').css('margin-top'))*3;
	if(board.layout<1){
		...
		var width=el.width();
		var catlistWidth=(width+21)*(jQuery('.catlist').length+1)+15;
		...
		jQuery('#taskboard_container').css('height',catlistHeight+parseInt(jQuery('.catlist').css('margin-top'))*3);
	}else{
		jQuery('#taskboard_container').css('height',catlistHeight+parseInt(jQuery('.catlist').css('margin-top'))*3);
	}
...
function init_catlist_add(){
	...
	jQuery('.cat-add').click(function(){
		jQuery(this).addClass('hide');jQuery('.form-catlist-add').removeClass('hide');
		if(board.layout>0){
			jQuery('#taskboard_container').scrollTop(jQuery('#catlist_container').outerHeight(true));
		}
		jQuery(document).on('mousedown.catlist-new',function(e){
			if(jQuery(e.target).closest('.catlist-new').length<1){
				jQuery('.catlist-new button[data-click=cancel]').trigger('click');
			}
		});
		return false;
	});
	el.find('button').click(function(){
		switch(jQuery(this).data('click')){
			case 'cancel':
				el.find('.btn-success').button('reset');el.find('.form-catlist-add').addClass('hide');jQuery('.cat-add').removeClass('hide');jQuery(document).off('mousedown.catlist-new');return false;
			case 'submit':
...
function add_catlist(catname){
	var el=jQuery('.catlist-new');
	el.find('.btn-success').button('loading');
...
function appendCatlist(data){
 var html='';
     ...
     // html+='      <div class="catlist_cont">';
     html+='          <div class="catlist-header">';
	 ...
	 html+='					<button data-click="submit" class="btn btn-success confirm mr10" data-loading-text="保存中..." >保存 </button>'; 
     ...
     html+='              <a href="javascript:;" class="task-add" catid="'+data.catid+'"><span class="dzz dzz-add"></span>新建任务</a>';
     html+='     </div>';
...
function init_task_poster(){
	jQuery(document).on('click','.task-add',function(){
		...
		jQuery(document).on('mousedown.task-poster',function(e){
			if(jQuery(e.target).closest('.task-poster').length<1){
				jQuery('.task-poster button[data-click=cancel]').trigger('click');
	...
	jQuery(document).on('click','.task-poster button',function(){
		...
		switch(jQuery(this).data('click')){
			case 'cancel':
				el.find('.btn-success').button('reset');el.addClass('hide').find('.task-item-badges').empty().end().find('.task-item-labels').empty().end().find('.task-item-members').empty();jQuery('.task-add').removeClass('hide');jQuery(document).off('mousedown.task-poster');
...
function appendTask(data,taskid){
	...
	html+='<div class="task-item" id="task_'+data.taskid+'" taskid="'+data.taskid+'">';
	...
	html+='<div class="task_item_edit"> ';
	html+='			<a href="javascript:;" class="task-item-menu action-icon js-popbox" data-href="'+MOD_URL+'&op=menu&do=taskmenu&taskid='+data.taskid+'" data-taskid="'+data.taskid+'" data-placement="right" data-auto-adapt="true"><span class="dzz dzz-create"></span></a>';
	html+='</div> ';
	html+=' <div class="task-item-main">';
	html+='	 	<a class="task-item-check" data-taskid="'+data.taskid+'"><span class="checkbox-custom checkbox-success"><input type="checkbox" name="" value="2" onchange="task_complete(\''+data.taskid+'\')"><label></label></span></a>';
	...
	for(var i in data.user_assign){
		html+='		 <a class="avatar avatar-30 js-popbox" data-href="'+MOD_URL+'&op=menu&do=taskmenu&uid='+data.user_assign[i].uid+'&taskid='+taskid+'&step=15&ac=2" data-placement="top" data-auto-adapt="true" title="'+data.user_assign[i].username+'" uid="'+data.user_assign[i].uid+'" href="javascript:;"> ';
...
function edit_hover(){
	jQuery('.task-item-cont').mouseover(function(){
        jQuery(this).find('.task_item_edit').addClass('task_item_edit_hover');
    })
    jQuery('.task-item-cont').mouseleave(function(){
        jQuery(this).find('.task_item_edit').removeClass('task_item_edit_hover');
    })
    jQuery('.task_item_edit a').click(function(){
        var el=jQuery(this).closest('.task_item_edit');
        el.addClass('task_item_edit_hover1');
        jQuery('.task_item_edit').not(el).removeClass('task_item_edit_hover1');
    })
    jQuery(document).click(function(){
        jQuery('.task_item_edit').removeClass('task_item_edit_hover1');
    })
}
...
function task_toggle_label(taskid,pow,obj){//选择标签
	...
	if(flag){
		...
		if(!jQuery('#task_'+taskid+' .task-item-labels .task-label[pow='+pow+']').length){
			...
			jQuery('.task-panel[taskid='+taskid+'] .task-labels .list-unstyled .task_labels_btn').before('<li class="task-label '+color+'-label" pow="'+pow+'">'+labelname+'</li>');
...
function task_toggle_follow_user(taskid,uid,username,obj){ //分配任务 
	...
	if(flag){
		...
		if(jQuery('#task_panel_'+taskid).length){
			var html1='<li><a class="avatar avatar-40 js-popbox" data-href="'+MOD_URL+'&op=menu&do=taskmenu&uid='+uid+'&taskid='+taskid+'&step=15&ac=1" data-placement="top" data-auto-adapt="true" title="'+username+'" uid="'+uid+'" href="javascript:;">';
				html1+='</a></li>';
			var el2=jQuery(html1);
			jQuery('#task_panel_'+taskid+' .task-members.follow ul .task_labels_btn').before(el2);
			el.find('.Topcarousel,img.img-circle').clone().appendTo(el2.find('.avatar'));
		}
	}
...
function task_archive(taskid){//归档完成任务
	...
	jQuery.getJSON(correcturl(cpurl+'&do=taskarchive'),{'taskid':taskid,'taskids':[taskid]},function(data){
		if(data.msg=='success'){
			...
			layout_catlist(jQuery('#catlist_'+catid));
			console.log(jQuery('#catlist_'+catid));
			task_panel_close();
		}
	});
}
...
function appendTaskTodo(data){
	...
	html+='		</div>';
	html+='		<button onclick="task_save_todo(\''+data.subid+'\',\''+data.taskid+'\')" data-loading-text="保存"  class="btn btn-success todo-save">保存 </button>';
	html+='		<button class="btn btn-link todo-del" onclick="task_del_todo(\''+data.subid+'\', \''+data.taskid+'\')"> 删除 </button>';
	html+='	  </div>';
	html+='	</div>';
	html+='  </li>';
	jQuery(html).appendTo('#task_panel_'+data.taskid+' .task-todos .sortable');
	task_subs_update(data.taskid);
	jQuery('#taskpanel .sortable').dragsort({
		'scrollContainer':jQuery('#taskpanel .task-panel-body'), //滚动层
		'contentContainer':jQuery('#taskpanel .task-panel-wrapper') //内容层
	...
	jQuery('#catlist_'+catid+' .task-item').each(function(){
		...
		if(flag){
			if(jQuery('#task_'+taskid+' .task-item-labels .task-label[pow='+pow+']').length){
				jQuery('#task_'+taskid+' .task-item-labels .task-label[pow='+pow+']').remove();
				jQuery('.task-panel[taskid='+taskid+'] .task-labels .task-label[pow='+pow+']').remove();
				jQuery.get(ccorrecturl(purl+'&do=setLabel'),{"pow":pow,"taskid":taskid,'isadd':0},function(){});
			}
		}else{
			if(!jQuery('#task_'+taskid+' .task-item-labels .task-label[pow='+pow+']').length){
				jQuery('<div class="task-label '+color+'-label" pow="'+pow+'"></div>').appendTo('#task_'+taskid+' .task-item-labels');
...
function cat_complete_archive(catid){//归档完成任务
	var taskids=[];
	jQuery('#catlist_'+catid+' .task-item-check .checkbox-custom input:checked').each(function(){
...
function showTaskPanel(taskid){
	showWindow('view_'+taskid,MOD_URL+'&op=view&taskid='+taskid+'&inajax=1','get',1);
}
...
function task_attach_down(qid){
	var url=MOD_URL+'&op=down&qid='+qid;
	if(BROWSER.ie){
			window.open(url);
		}else{
...
function task_attach_saveto(qid){
	var url=MOD_URL+'&op=saveto&qid='+qid;
	if(!window.frames['hidefram']) jQuery('<iframe id="hideframe" name="hideframe" src="about:blank" frameborder="0" marginheight="0" marginwidth="0" width="0" height="0" allowtransparency="true" style="display:none;z-index:-99999"></iframe>').appendTo('body');
	window.frames['hideframe'].location=url;
	
}
function task_attach_preview(qid){
	var url=MOD_URL+'&op=preview&qid='+qid;
...
function board_complete_archive(tbid){//归档所有完成任务
	var taskids=[];
	showDialog('确定要归档所有已完成的任务？<br><small>归档后的任务可以在已归档内找到，并且可以再次激活</small>','confirm','',function(){
		var el=jQuery('.task-item-check input:checked');
		el.each(function(){
			taskids.push(jQuery(this).parent().data('taskid'));
		});
		if(taskids.length){
			jQuery.getJSON( correcturl(cpurl+'&do=taskarchive'),{'taskids':taskids},function(data){
				if(data.msg=='success'){
...
function board_filter(){ //过滤结果
	...
	if(filter.metype!=''){
		...
		jQuery('.task-item').each(function(){
				...
				switch(filter.datetype){
					...
					case 'nextweek':
						if(!jQuery(this).find('.task-badge-time').length){ 
							...
							week_u_stamp=week_u.getTime();
							//console.log([week_l.format('yyyy-MM-dd'),week_u.format('yyyy-MM-dd'),jQuery(this).find('.task-badge-time').attr('time')]);
							var taskdate=new Date(jQuery(this).find('.task-badge-time').attr('time')).getTime();
							if(taskdate<week_l || taskdate>week_u){
								hide=1;	
							}
					...
					case 'month':
						if(!jQuery(this).find('.task-badge-time').length){ 
							...
							//console.log([month_l.format('yyyy-MM-dd'),month_u.format('yyyy-MM-dd'),jQuery(this).find('.task-badge-time').attr('time')]);
							var taskdate=new Date(jQuery(this).find('.task-badge-time').attr('time')).getTime();
			...
			//按名称查询
			if(filter.keyword!=''){
				//console.log([jQuery(this).find('.task-item-title').html(),filter.keyword]);
...
function board_setting_archive(tbid){//归档任务版
	showDialog('确定要归档此任务板？<br><small>归档后的任务板任何人都不能做修改操作；可以在已归档内找到，并且可以再次激活</small>','confirm','',function(){
			jQuery.getJSON(correcturl(ajaxurl+'&do=setting&operation=archive'),{'tbid':tbid},function(data){
				if(data.msg=='success'){
					showmessage('归档成功！正在刷新页面...','success',3000,1,'center',function(){
						window.location.reload();
					});
				}else{
					showmessage(json.error,'danger',3000,1,'center');
				}
			});
	},1);
}
function board_setting_active(tbid){//归档任务版
	showDialog('确定要激活此任务板？<br><small>激活后的任务板将恢复正常状态，成员可以进行修改操作</small>','confirm','',function(){
			jQuery.getJSON(correcturl(ajaxurl+'&do=setting&operation=active'),{'tbid':tbid},function(data){
				if(data.msg=='success'){
					showmessage('激活成功！正在刷新页面...','success',3000,1,'center',function(){
						window.location.reload();
					});
				}else{
					showmessage(json.error,'danger',3000,1,'center');
...
function board_setting_restore(tbid){//归档任务版
	showDialog('确定要恢复此任务板？<br><small>恢复后的任务板将恢复正常状态，成员可以进行修改操作</small>','confirm','',function(){
			jQuery.getJSON(correcturl(ajaxurl+'&do=setting&operation=restore'),{'tbid':tbid},function(data){
				if(data.msg=='success'){
					showmessage('恢复成功！正在刷新页面...','success',3000,1,'center',function(){
						window.location.reload();
					});
				}else{
					showmessage(json.error,'danger',3000,1,'center');
...
function board_setting_delete(tbid){//归档任务版
	showDialog('确定要删除此任务板？<br><small>删除后的任务板可以在回收站内找到</small>','confirm','',function(){
			jQuery.getJSON(correcturl(ajaxurl+'&do=setting&operation=delete'),{'tbid':tbid},function(data){
				if(data.msg=='success'){
					showmessage('删除成功！正在刷新页面...','success',3000,1,'center',function(){
						window.location.reload();
					});
				}else{
					showmessage(json.error,'danger',3000,1,'center');
...
function board_recycle_empty(tbid){//清空任务板回收站
	showDialog('此操作将清空回收站内所有任务、列表和文件，且不可恢复<br>您确定要清空回收站吗？','confirm','',function(){
			jQuery.getJSON(correcturl(ajaxurl+'&do=recycle&operation=empty'),{'tbid':tbid},function(data){
				if(data.msg=='success'){
					showmessage('清空完成！正在刷新页面...','success',3000,1,'center',function(){
						window.location.reload();
					});
				}else{
					showmessage(json.error,'danger',3000,1,'center');
