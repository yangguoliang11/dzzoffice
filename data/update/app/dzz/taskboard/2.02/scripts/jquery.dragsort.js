...
(function($){
  $.fn.dragsort = function(options,recall) {
	var opt={
		'scrollContainer':$('#taskboard_container'), //滚动层
		'contentContainer':$('#catlist_container'), //内容层
	...
	$(container).children().off("mousedown.subdrager").on('mousedown.subdrager',function(e) {
		...
		$(document).on('mousemove.subdrager',function(e) {
			...
			// 遍历所有块的坐标
			$(container).children().not(_this).not(wid).not('.nodrager').each(function(i) {
				var obj = $(this);
				var p = obj.offset();
				var a3 = p.top;
	...
	var PreMove=function() {
	    ...
	    window.kp_only=true;
		jQuery('#taskpanel_blank').empty().show();
