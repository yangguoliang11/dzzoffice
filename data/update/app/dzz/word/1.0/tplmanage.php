...
if ($do == 'cat_rename') {
	...
	$cid = intval($_GET['cid']);
	$tid = intval($_GET['tid']);
	$cat = C::t('doc_template_cat')->fetch(array('cid' => $cid, 'type' => 1));
	...
	if(submitcheck('savebg')){
		...
		exit(json_encode(array('code' => 200, 'src' => C::t('attachment')->getThumbByAid($aid,144,183))));
	} else {
	...
	if(submitcheck('addtpl')){
		$cid = intval($_GET['cid']);
		if (!C::t('doc_template_cat')->fetch(array('cid' => $cid, 'type' => 1))) exit(json_encode(array('code' => 400, 'message' => lang('chose_cat_error'))));
		$setarr = array(
					'title' 		=>		 $_GET['name'],
					'attach'		=>		 $_GET['aid'],
	...
	$name = $_GET['name'];
	if (DB::result_first('select count(*) from %t where name = %s and type = 1', array('doc_template_cat', $name))) {
