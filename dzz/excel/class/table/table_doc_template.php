...
class table_doc_template extends dzz_table
{
	...
	{
		$data = DB::fetch_all('select t.* from %t t left join %t c on t.cat_id = c.cid where c.type = 2 group by t.tid order by t.time', array($this->_table, 'doc_template_cat'), 'tid');
