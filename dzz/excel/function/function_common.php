...
{
	$uid = $uid ? $uid : $_G['uid'];
	$appid = C::t('user_setting')->fetch_by_skey('doc_excel_openappid', $uid);
	$openappid = $appid;
	if (!$appid) {
		$openappid = DB::result_first('select appid from %t where ext IN(%n) order by isdefault desc, disp asc', array('app_open', array('xls', 'xlsx')));
	}
	$app = C::t('app_market')->fetch_by_appid($openappid);
	$savepath = C::t('user_setting')->fetch_by_skey('doc_excel_savepath', $uid);
	if (!$savepath) {
	...
	$tplhide = C::t('user_setting')->fetch_by_skey('doc_excel_tplhide', $uid);
	$createtype = C::t('user_setting')->fetch_by_skey('doc_excel_createtype', $uid);
	return $setting = array(
						'showtype'		 =>		 C::t('user_setting')->fetch_by_skey('doc_excel_showtype', $uid),
