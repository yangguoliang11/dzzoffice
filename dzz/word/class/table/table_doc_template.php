...
class table_doc_template extends dzz_table
{
	...
	{
		$data = DB::fetch_all('select t.* from %t t left join %t c on t.cat_id = c.cid where c.type = 1 group by t.tid order by t.time', array($this->_table, 'doc_template_cat'), 'tid');
		foreach ($data as $key => $value) {
			if ($value['cover_aid']) {
				$data[$key]['img'] = C::t('attachment')->getThumbByAid($value['cover_aid'],144,183);
			} else {
