...
class table_task_setting extends dzz_table
{
	public function __construct() {

		$this->_table = 'task_setting';
		$this->_pk    = 'skey';
	...
	//获取封面数组
	public function getCovers($width=64,$height=64,$original=1) {
		if($cover=self::fetch('cover')){
			$aids=explode(',',$cover);
		}
		$return=$data=array();
		if($aids){
			foreach(C::t('attachment')->fetch_all($aids) as $value){
				$value['img']=C::t('attachment')->getThumbByAid($value,$width,$height,$original);
